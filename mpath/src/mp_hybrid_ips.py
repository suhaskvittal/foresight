...
class MPATH_HYBRID_IPS(TransformationPass):
    ...
    def run(self, dag):
        ...
        original_layer_size = len(primary_layer_view)
        self.router_usage = defaultdict(int)
        ...
        exec_rest = len(primary_layer_view) > 0
        if exec_rest:  # We exited because we are switching to SABRE.
            min_size = -1
            min_depth = -1
            min_layout = None
            min_dag = None
            ...
            for (layout, soln, size) in solutions:
                ...
                for i in range(len(primary_layer_view)):
                    for node in primary_layer_view[i]:
                        mapped_node = self._remap_gate_for_layout(node, layout, canonical_register)
                        post_dag.apply_operation_back(op=mapped_node.op, qargs=mapped_node.qargs, cargs=mapped_node.cargs)
                    for node in secondary_layer_view[i]:
                        mapped_node = self._remap_gate_for_layout(node, layout, canonical_register)
                        post_dag.apply_operation_back(op=mapped_node.op, qargs=mapped_node.qargs, cargs=mapped_node.cargs)
                post_dag = sabre.run(post_dag)
                size = post_dag.size() + size
                depth = len(soln) + post_dag.depth()
                if min_size < 0 or size < min_size or (size == min_size and depth < min_depth): 
                    min_layout = sabre.property_set['final_layout'] 
                    min_size = size
                    min_depth = depth
                    min_dag = post_dag
                    min_router_usage = copy(sabre.router_usage)
            # Update router usage
            self.router_usage['sabre'] += min_router_usage['sabre']
            self.router_usage['ips'] += min_router_usage['ips']
            hybrid_dag = min_dag
        ...
        if exec_rest:
            ...
            mapped_dag.compose(hybrid_dag, inplace=True)
        return mapped_dag
