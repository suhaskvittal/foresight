...
def profile_optimal_workload(method, out_file, coupling_map, runs=100):
    data = defaultdict(list)    

    benchmark_pass = BenchmarkPass(coupling_map, None, compare=['sabre', 'ips'], runs=1, compute_stats=True)
    ...
    elif method == 'zulehner':
        suite = G_ZULEHNER
        for qb_file in suite:
            circ = QuantumCircuit.from_qasm_file('benchmarks/zulehner/%s' % qb_file)
            if circ.depth() > 500 or circ.depth() < 30:
                continue
            _pad_circuit_to_fit(circ, coupling_map)
            print('[%s]' % qb_file)
...
if __name__ == '__main__':
    ...
    profile_optimal_workload(method, out_file, coupling_map)
